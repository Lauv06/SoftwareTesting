  ...
  <div id="telChargeDe">
    <el-collapse v-model="activeNames" accordion>
      <el-collapse-item name="1">
        ...
        <el-form status-icon label-width="140px" ref="inputs" :model="inputs" :rules="rules" style="width: 500px;margin: auto">
          <el-form-item label="本月通话分钟数" prop="minutes">
            <el-input v-model.number="inputs.minutes"></el-input>
          </el-form-item>
          <el-form-item label="未按时缴费次数" prop="times">
            <el-input v-model.number="inputs.times"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="submitForm('inputs')">提交</el-button>
            <el-button @click="resetForm('inputs')">重置</el-button>
          </el-form-item>
          <el-form-item label="通话总费用">
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.get('http://127.0.0.1:8000/api/telephone', {params: this.inputs})
              .then((response) => {
    ...
    startTesting() {
      this.$http.get('http://127.0.0.1:8000/api/telephoneTest',
